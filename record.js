...
exports.handler = async (event) => {
  ...
  // ── GET: return all picks + record summary ─────────────────────────────────
  if (method === "GET") {
    try {
      const raw = await store.get("all_picks", { type: "text" });
      ...
      return {
        ...
        body: JSON.stringify({ picks, record }),
      };
    } catch (err) {
  ...
  // ── POST: save today's top plays ───────────────────────────────────────────
  if (method === "POST") {
    try {
      const { plays } = JSON.parse(event.body);
      const raw = await store.get("all_picks", { type: "text" }).catch(() => null);
      ...
      return {
        statusCode: 200,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ saved: newPicks.length, date: today }),
      };
    } catch (err) {
      return { statusCode: 500, body: JSON.stringify({ error: err.message }) };
    }
  ...
  // ── PUT: grade yesterday's picks against scores ────────────────────────────
  if (method === "PUT") {
    ...
    try {
      ...
      const pendingPicks = picks.filter(p => p.result === "P");
      if (pendingPicks.length === 0) {
        return { statusCode: 200, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ graded: 0, message: "No pending picks" }) };
      }
      ...
      const updated = picks.map(pick => {
        ...
        let result = "P";
        if (pick.type === "RUN LINE") {
          result = (favWon && margin >= 2) ? "W" : "L";
      ...
      return {
        statusCode: 200,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ graded, total: picks.length }),
      };
    } catch (err) {
      return { statusCode: 500, body: JSON.stringify({ error: err.message }) };
    }
